...
WITH RECURSIVE
  target(symbol) AS (VALUES ('il2cpp::vm::Class::Init')),
  target_node(callsite_id) AS (
    ...
    JOIN target AS t
    WHERE
      IFNULL(f.deobfuscated_name, '') GLOB '*' || t.symbol || '*'
  ...
  matched_callsite(leaf_callsite_id) AS (
    ...
    SELECT callsite_id FROM target_node
    UNION ALL
    SELECT child.id
    FROM matched_callsite AS parent
    JOIN stack_profile_callsite AS child
      ON child.parent_id = parent.leaf_callsite_id
  ),
  alloc_group AS (
  ...
  stack_walk(leaf_callsite_id, callsite_id, depth, frame_name, parent_id) AS (
    ...
    JOIN stack_profile_callsite AS cs
      ON cs.id = ag.callsite_id
    JOIN stack_profile_frame AS f
      ON f.id = cs.frame_id
    UNION ALL
      ...
      COALESCE(
        (
          ...
          FROM (
            ...
            FROM stack_profile_symbol AS s
            WHERE
              s.symbol_set_id = pf.symbol_set_id
            ORDER BY
              s.id
          ) AS symbol_ordered
        ),
    ...
    JOIN stack_profile_callsite AS parent
      ON parent.id = sw.parent_id
    JOIN stack_profile_frame AS pf
      ON pf.id = parent.frame_id
  )
SELECT
  ag.upid,
  p.pid,
  ...
  (
    ...
    FROM (
      ...
      FROM stack_walk AS sw
      WHERE
        sw.leaf_callsite_id = ag.callsite_id
      ORDER BY
        sw.depth DESC
    ) AS ordered
  ) AS stack
