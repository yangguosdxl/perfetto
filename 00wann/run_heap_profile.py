...
MALLOC_LIVE_SQL = (
    "select coalesce(sum(size), 0) as malloc_live_bytes "
...
def load_perfetto_root(script_dir: Path) -> Path:
    ...
    if proc.returncode != 0 or not proc.stdout:
        print(f"HEAP_PROFILE_FAILED|reason=load_config_failed|stderr={proc.stderr.strip()}")
        raise SystemExit(1)
...
def query_trace_value(trace_processor: Path, query: str, trace_path: Path) -> str:
    proc = subprocess.run(
        [str(trace_processor), "-Q", query, str(trace_path)],
        stdout=subprocess.PIPE,
...
def write_validation(validation_path: Path, lines: list[str], append: bool = False) -> None:
...
def validate_heap_profile_against_meminfo(
    ...
    if not meminfo_path.exists() and not capture_meminfo_snapshot(out_dir, package_name):
        write_validation(
            validation_path,
    ...
    malloc_live_bytes_text = query_trace_value(trace_processor, MALLOC_LIVE_SQL, trace_path)
    ...
    allowed_diff_bytes = int(os.environ.get("HEAP_PROFILE_MEMINFO_ALLOWED_DIFF_BYTES", "67108864"))
...
def main(argv: list[str]) -> int:
    ...
    out_dir = script_dir / "PerfData/mem" / _datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    out_dir.mkdir(parents=True, exist_ok=True)

    if source_fsbootcmd(script_dir, env) != 0:
        return 1

    run(["adb", "push", "debugconfig.txt", f"/sdcard/Android/data/{APP}/files"], env=env)
    ...
    profile_log = Path(tempfile.gettempdir()) / f"run_heap_profile_{int(time.time())}_{os.getpid()}.log"
    ...
    pump_thread = threading.Thread(target=logger.pump, args=(profiler_proc,), daemon=True)
    pump_thread.start()

    active_timeout_s = int(os.environ.get("HEAP_PROFILE_ACTIVE_TIMEOUT_S", "60"))
    if not wait_for_log_pattern(logger, profiler_proc, "Profiling active", active_timeout_s):
        run(["cp", str(profile_log), str(profile_log_dest)], env=env)
        print(f"HEAP_PROFILE_FAILED|reason=profiling_active_timeout|out_dir={out_dir}|log={profile_log_dest}")
    ...
    shutdown_timeout_s = int(os.environ.get("HEAP_PROFILE_SHUTDOWN_SIGNAL_TIMEOUT_S", "600"))
    if wait_for_log_pattern(logger, profiler_proc, "Waiting for profiler shutdown", shutdown_timeout_s):
    ...
    if not meminfo_captured and ((out_dir / "raw-trace").exists() or (out_dir / "symbolized-trace").exists()):
        capture_meminfo_snapshot(out_dir, APP)

    validation_rc = validate_heap_profile_against_meminfo(out_dir, APP, trace_processor)
